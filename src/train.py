...
This script trains a Sentence Transformer model and stores embeddings in Supabase.
"""
...
def load_movie_data_from_supabase(config: Dict[str, Any]) -> pd.DataFrame:
    """
    Load all movie data from a Supabase table, handling pagination.
    ...
    while True:
        logger.info(f"Fetching movies from position {current_pos} to {current_pos + page_size - 1}...")
        response = (
    ...
    if all_movies:
        logger.info(f"Loaded a total of {len(all_movies)} movies from Supabase.")
        return pd.DataFrame(all_movies)
    else:
        logger.info("No movies found in Supabase.")
        return pd.DataFrame()
...
def main():
    ...
    mlflow.set_experiment("Movie Embedding Generation")

    # Start an MLflow run. All parameters, metrics, and artifacts will be logged to this run.
    with mlflow.start_run():
        start_time = time.time()
        logger.info("Starting movie recommendation model training and embedding process.")
        ...
        try:
            movies_df = load_movie_data_from_supabase(config)
        except Exception as e:
        ...
        # Log the saved model and other artifacts from the output directory
        logger.info(f"Logging artifacts from '{output_dir}' to MLflow.")
        if os.path.exists(output_dir) and os.path.isdir(output_dir):
            mlflow.log_artifacts(output_dir, artifact_path="model_artifacts")
        
        total_duration = time.time() - start_time
        mlflow.log_metric("total_duration_sec", round(total_duration, 2))
        mlflow.log_metric("run_status", 1) # Log success
        
        logger.info("Model training and embedding storage completed successfully.")
